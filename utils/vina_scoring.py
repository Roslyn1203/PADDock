...
def run_vina_api_worker(args):
    """
    Worker: compute Vina score and RMSD, and return smoothed reward values.
    """
    mol_idx, complex_name, lig_mol, pos_tensor, gt_pos_tensor, receptor_pdbqt_path, original_center_tensor = args
    
    # Default fallback values
    default_score = 15.0  # Vina is usually in [-10, 0], 15 is a very poor score
    default_rmsd = 20.0   # 20A is a strong failure penalty for RL
    ...
    try:
        ...
        # === 3. Vina computation (with RMSD gating) ===
        score = default_score
        
        # If RMSD is already > 10.0, skip Vina to save compute and avoid unstable failures
        if rmsd_val <= 10.0:
            try:
                ...
                # Keep failure handling quiet to avoid log flooding
                score = default_score

        # === 4. Smooth reward shaping ===
        
        # A. Process Vina score
        if score > 10.0:
            score_final = 10.0 + np.log1p(score - 10.0)
        else:
            score_final = score

        # B. Process RMSD score
        if rmsd_val > 15.0:
            rmsd_final = 15.0 + np.log1p(rmsd_val - 15.0)
        else:
            rmsd_final = rmsd_val

        return score_final, rmsd_final
