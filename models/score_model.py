...
class TensorProductScoreModel(torch.nn.Module):
    ...
    def forward(self, data):
        ...
        for l in range(len(self.lig_conv_layers)):
            ...
            # Optimization 1: pass layer index explicitly instead of implicit closure capture
            # Also pass graph indices explicitly to avoid autograd issues from closures
            def run_layer_block(lig_n_attr, rec_n_attr,
                                lig_e_attr, rec_e_attr, cross_e_attr,
                                lig_e_sh, rec_e_sh, cross_e_sh,
                                layer_idx): 
                
                # Ligand updates
                lig_edge_attr_ = torch.cat([lig_e_attr, lig_n_attr[lig_src, :self.ns], lig_n_attr[lig_dst, :self.ns]], -1)
                lig_intra_update = self.lig_conv_layers[layer_idx](lig_n_attr, lig_edge_index, lig_edge_attr_, lig_e_sh)

                rec_to_lig_edge_attr_ = torch.cat([cross_e_attr, lig_n_attr[cross_lig, :self.ns], rec_n_attr[cross_rec, :self.ns]], -1)
                lig_inter_update = self.rec_to_lig_conv_layers[layer_idx](rec_n_attr, cross_edge_index, rec_to_lig_edge_attr_, cross_e_sh,
                                                                out_nodes=lig_n_attr.shape[0])

                # Receptor updates
                if layer_idx != len(self.lig_conv_layers) - 1:
                    rec_edge_attr_ = torch.cat([rec_e_attr, rec_n_attr[rec_src, :self.ns], rec_n_attr[rec_dst, :self.ns]], -1)
                    rec_intra_update = self.rec_conv_layers[layer_idx](rec_n_attr, rec_edge_index, rec_edge_attr_, rec_e_sh)

                    lig_to_rec_edge_attr_ = torch.cat([cross_e_attr, lig_n_attr[cross_lig, :self.ns], rec_n_attr[cross_rec, :self.ns]], -1)
                    rec_inter_update = self.lig_to_rec_conv_layers[layer_idx](lig_n_attr, torch.flip(cross_edge_index, dims=[0]), lig_to_rec_edge_attr_,
                                                                    cross_e_sh, out_nodes=rec_n_attr.shape[0])
                else:
                    rec_intra_update, rec_inter_update = None, None

                # Padding & Residual
                lig_node_attr_out = F.pad(lig_n_attr, (0, lig_intra_update.shape[-1] - lig_n_attr.shape[-1]))
                lig_node_attr_out = lig_node_attr_out + lig_intra_update + lig_inter_update

                if layer_idx != len(self.lig_conv_layers) - 1:
                    rec_node_attr_out = F.pad(rec_n_attr, (0, rec_intra_update.shape[-1] - rec_n_attr.shape[-1]))
                    rec_node_attr_out = rec_node_attr_out + rec_intra_update + rec_inter_update
                else:
                    rec_node_attr_out = rec_n_attr

                return lig_node_attr_out, rec_node_attr_out

            # Run checkpoint
            # Enable checkpointing only in training mode with grad-enabled node features
            if self.training and (lig_node_attr.requires_grad or rec_node_attr.requires_grad):
                # Core fix: use_reentrant=False is critical for OOM stability
                lig_node_attr, rec_node_attr = checkpoint(
                    run_layer_block,
                    lig_node_attr, rec_node_attr,
                    lig_edge_attr, rec_edge_attr, cross_edge_attr,
                    lig_edge_sh, rec_edge_sh, cross_edge_sh,
                    layer_idx=l,          # pass non-tensor argument via kwargs
                    use_reentrant=False   # must remain False
                )
            else:
                lig_node_attr, rec_node_attr = run_layer_block(
                    lig_node_attr, rec_node_attr,
                    lig_edge_attr, rec_edge_attr, cross_edge_attr,
                    lig_edge_sh, rec_edge_sh, cross_edge_sh,
                    layer_idx=l
