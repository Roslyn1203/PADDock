...
def main():
    ...
    # New output folder (e.g., evaluation every 5 epochs)
    eval_tag = "val_every5"

    csv_out_dir = os.path.join(rl_ckpts_dir, f'{eval_tag}_csv_results')
    summary_csv_path = os.path.join(rl_ckpts_dir, f'{eval_tag}_SUMMARY.csv')
    ...
    pt_files = glob.glob(os.path.join(rl_ckpts_dir, 'rl_model_epoch_*.pt'))
    all_epochs = sorted([int(f.split('_')[-1].split('.')[0]) for f in pt_files])

    # Keep only every 5th epoch
    epochs_to_eval = [e for e in all_epochs if e % 5 == 0]
