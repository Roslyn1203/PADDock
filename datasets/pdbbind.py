...
from torch.utils.data.distributed import DistributedSampler # import distributed sampler

class InitTransform(BaseTransform):
    """
    Transform designed specifically for RL.
    Purpose:
    1. Select one conformation from the list as initial reference.
    2. Initialize required time-related attributes.
    3. Do not add noise to keep data clean.
    """
    ...
    def __call__(self, data):
        # 1. [Key] Handle multi-conformation cases
        # If pos is a list, pick a deterministic reference conformation.
        if not torch.is_tensor(data['ligand'].pos):
            data['ligand'].pos = data['ligand'].pos[0] 

        # 2. [Key] Initialize time attributes
        # DataLoader batches must include these keys for consistent collation.
        t = 0.0 
        t_tr, t_rot, t_tor = t, t, t
        
        # Compute dummy sigma for set_time interface compatibility
        tr_sigma, rot_sigma, tor_sigma = self.t_to_sigma(t_tr, t_rot, t_tor)
        
        # This adds node_t, edge_t, and related attributes
        set_time(data, t_tr, t_rot, t_tor, 1, self.all_atom, device=None)

        # 3. Clear supervision signals not needed for RL
        # NoiseTransform computes tr_score etc., which are not required in RL
        data.tr_score = torch.zeros(1, 3) # Dummy
        # ... other score fields are not required by RL loss
...
def construct_loader(args, t_to_sigma, device):
    # [Modified] Use InitTransform
    transform = InitTransform(t_to_sigma=t_to_sigma, no_torsion=args.no_torsion,
    ...
    common_args = {
        ...
        # [Key fix] This must be enabled, otherwise dataset will not load RDKit molecules
        'require_ligand': True  
    }
    ...
    # [Key fix 2] Use standard DataLoader
    # Output will be torch_geometric.data.Batch, directly compatible with sampling_rl
    
    # [Key fix 3] Adapt to multi-GPU distributed training (DDP)
    # Check whether distributed environment is initialized
    ...
    train_loader = DataLoader(
        ...
        shuffle=(train_sampler is None), # if sampler is used, shuffle should be controlled by sampler
